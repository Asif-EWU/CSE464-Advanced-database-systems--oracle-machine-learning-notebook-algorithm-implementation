...
BEGIN
    ...
    v_setlst('GLMS_DIAGNOSTICS_TABLE_NAME') := 'GLMR_SH_SAMPLE_DIAG_SUPPORT';
...
-- 5
CREATE OR REPLACE VIEW survival_prediction_support AS
    SELECT passengerid, round(PREDICTION(SUPPORT_VECTOR_MACHINES USING *)) PREDICTION_SURVIVED, SURVIVED ACTUAL_SURVIVED
...
BEGIN
    ...
    FROM survival_prediction_support;
    
    select count(*) into correct_row
    from survival_prediction_support a
